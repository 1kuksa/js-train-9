...
else return "рядок Числа яке задовільняє умову не знайдено"
	}
...
function getPositiveArrayValues(arr) {
	if (Array.isArray(arr)) {
		const iter =  Array.from(arr.values())
		
		const positiveArrayValues = iter.filter(element => element > 0);
...
function removeAndDouble(arr, startIdx) {
	if (Array.isArray(arr)) {
		...
		const doubledArray = arr.map((element, index, array) => {
			return element * 2
	})
    return doubledArray}
...
function sumAndPrint(arr) {
	if (Array.isArray(arr)) {
		let initialValue = 0;
		const sum = arr.reduce((num, index) => num + index, initialValue)
...
function flattenSquareAndFilter(arr, threshold) {
	if (Array.isArray(arr)) {
		...
		const arrMap = arrFlat.map((element, index, array) => {
			return element * element
	});
	const arrFilter = arrMap.filter(element => element > threshold);
	return arrFilter
	}
...
function getLastElementsSumAndPrint(arr, n) {
	...
	const sum = arrSlice.reduceRight((element, num) => element + num, 0)
	return sum
	}
...
function wordLengths(arr) {
	if (Array.isArray(arr)) {
		const lengthsArray = arr.map((element, index, array) => {
...
function findLongestWord(arr) {
	if (Array.isArray(arr)) {
		const longestWord = arr.reduce((element, word) => {return element.length > word.length ? element : word}, 0)
		return longestWord
   }
...
function capitalizeWords(arr) {
	if (Array.isArray(arr)) {
		const capitalizedArray = arr.map((word, index, array) => {
